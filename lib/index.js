(() => {
  ...
  const EBook = class {
    ...
    render () {
      ...
      EJS.renderFile(path.resolve(__dirname, '../templates/toc.ncx.ejs'), {
        ...this.options,
        content: this.content
      }, templateConfig, (_err, data) => {
        this.container.file('OEBPS/toc.ncx', data)
      })

      EJS.renderFile(path.resolve(__dirname, '../templates/epub3/toc.xhtml.ejs'), {
        ...this.options,
        content: this.content
      }, templateConfig, (_err, data) => {
      ...
      EJS.renderFile(path.resolve(__dirname, '../templates/epub3/content.opf.ejs'), {
        ...this.options,
        content: this.content
      }, templateConfig, (_err, data) => {
      ...
      this.content.forEach(chapter => {
        EJS.renderFile(path.resolve(__dirname, '../templates/epub3/chapter.xhtml.ejs'), {
          ...this.options,
          ...chapter
        }, templateConfig, (_err, data) => {
      ...
      this.container.file('OEBPS/style.css', fs.readFileSync(path.resolve(__dirname, '../templates/template.css')))
